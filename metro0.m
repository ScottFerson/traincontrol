...
figure;  plot(X,vtilde);
ann('Prescribed velocity profile','Location along track','velocity (m/s)');
figure;  plot(X,G);
ann('Slope of incline', 'Location along track', 'Gradient (%)');
...
figure;  plot(xx);
ann(['Total time ',num2str(t),' s'], 'Time', 'Position (m)');
figure;  plot(vv); hold on;  plot(vvv); 
ann('Train velocity', 'Time', 'Velocity (m/s)');
figure;  plot(FF); 
ann(['Total energy ', num2str(E)], 'Time', 'Motive force (N)');
toc;
end
